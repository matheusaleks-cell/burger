...
BEGIN
    ...
    -- 3. Insert New Drinks (Manual Upsert to avoid ON CONFLICT on non-unique column)
    ...
    IF EXISTS (SELECT 1 FROM products WHERE name = 'Água Mineral 500ml') THEN
        UPDATE products SET price = 5.00, availability_status = 'available', available_for_delivery = true, available_for_pousada = true WHERE name = 'Água Mineral 500ml';
    ELSE
        INSERT INTO products (name, description, price, category_id, is_active, availability_status, available_all, image_url, available_for_delivery, available_for_pousada)
    ...
    IF EXISTS (SELECT 1 FROM products WHERE name = 'Água com Gás 500ml') THEN
        UPDATE products SET price = 5.00, availability_status = 'available', available_for_delivery = true, available_for_pousada = true WHERE name = 'Água com Gás 500ml';
    ELSE
        INSERT INTO products (name, description, price, category_id, is_active, availability_status, available_all, image_url, available_for_delivery, available_for_pousada)
    ...
    IF EXISTS (SELECT 1 FROM products WHERE name = 'Água Mineral 1,5L') THEN
        UPDATE products SET price = 8.00, availability_status = 'available', available_for_delivery = true, available_for_pousada = true WHERE name = 'Água Mineral 1,5L';
    ELSE
        INSERT INTO products (name, description, price, category_id, is_active, availability_status, available_all, image_url, available_for_delivery, available_for_pousada)
    ...
    IF EXISTS (SELECT 1 FROM products WHERE name = 'Heineken Long Neck') THEN
        UPDATE products SET price = 12.00, availability_status = 'available', available_for_delivery = true, available_for_pousada = true WHERE name = 'Heineken Long Neck';
    ELSE
        INSERT INTO products (name, description, price, category_id, is_active, availability_status, available_all, image_url, available_for_delivery, available_for_pousada)
        VALUES ('Heineken Long Neck', 'Cerveja 330ml', 12.00, v_category_id, true, 'available', true, NULL, true, true);
    END IF;
    
    -- 4. Update Existing Products Images (Fuzzy Match)
    ...
    -- Combos
    UPDATE products SET image_url = '/products/burguer_do_now_DEFINITIVO/Big_Bacon_Now_+_Fritas_+_Coca-Cola_Lata.jpg'
    WHERE name ILIKE '%Combo%' AND name ILIKE '%Big Bacon%';

    UPDATE products SET image_url = '/products/burguer_do_now_DEFINITIVO/Big_Cheese_Now_+_Fritas_+_Coca-Cola_Lata.jpg'
    WHERE name ILIKE '%Combo%' AND name ILIKE '%Big Cheese%';

    UPDATE products SET image_url = '/products/burguer_do_now_DEFINITIVO/Cheese_Now_+_Fritas_+_Refrigerante_Lata.jpg'
    WHERE name ILIKE '%Combo%' AND name ILIKE '%Cheese Now%';

    UPDATE products SET image_url = '/products/burguer_do_now_DEFINITIVO/Now_Bacon_+_Fritas_+_Refrigerante_Lata.jpg'
    WHERE name ILIKE '%Combo%' AND name ILIKE '%Now Bacon%';

    UPDATE products SET image_url = '/products/burguer_do_now_DEFINITIVO/Combo_Casal_+_2_Cocas.jpg'
